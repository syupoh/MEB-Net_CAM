...
def extract_features(model, data_loader, unet=None, print_freq=10, metric=None):
    model.eval()
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None, unet=None,
                 cmc_flag=False, rerank=False, pre_features=None):
        if (pre_features is None):
            features, _ = extract_features(self.model, data_loader, unet=unet)
        else:
...
class JointEvaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None,
                 unet=None, cmc_flag=False, rerank=False, pre_features=None):
        if (pre_features is None):
            features, _ = extract_features(self.model, data_loader, unet=unet)
        else:
            features = pre_features
