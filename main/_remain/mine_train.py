...
def get_data2(name, data_dir, height, width, batch_size, workers, num_instances, iters=200):
    # root = osp.join(data_dir, name)
    root = data_dir
...
def create_model(args):
    ...
    arch.append(args.init_1.split('_')[3])
    arch.append(args.init_2.split('_')[3])
    arch.append(args.init_3.split('_')[3])
...
def main_worker(args):
    ...
    # # Create data source anda target loaders
    iters = args.iters if (args.iters>0) else None
    dataset_source, num_classes, train_loader_source, test_loader_source = \
        get_data2(args.dataset_source, args.data_dir, args.height,
                 args.width, args.batch_size, args.workers, args.num_instances, iters)

    dataset_target2, _, train_loader_target, test_loader_target2 = \
        get_data2(args.dataset_target, args.data_dir, args.height,
